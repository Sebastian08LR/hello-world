...
print_loading_bar() {
    ...
    for ((i = 0; i <= bar_length; i++)); do
        echo -ne "\033[1;35m\r[▄︻┻═┳一\033[00m"
        for ((j = 1; j <= bar_length; j++)); do
            if [ "$j" -le "$i" ]; then
                echo -ne "\033[0;32m➡️\033[00m"
            else
                echo -ne "\033[0;31m-\033[00m"
            fi
        done
        if [ "$((i * 100 / bar_length))" -lt 50 ]; then
            echo -ne "]\033[0;31m $((i * 100 / bar_length))% 🚧\033[00m"
        elif [ "$((i * 100 / bar_length))" -lt 100 ]; then
            echo -ne "]\033[0;33m $((i * 100 / bar_length))% 🚀\033[00m"
        else
            echo -ne "]\033[1;32m $((i * 100 / bar_length))% 🎉\033[00m"
        fi
